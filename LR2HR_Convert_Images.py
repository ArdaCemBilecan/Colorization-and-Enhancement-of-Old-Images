...
class Pix2Pix:
    ...
    def __init__(self,lr_img_path,hr_img_path,test_img_path, output_img_path, model_save_path,loss_object,epoch):
        self.lr_img_path = glob(lr_img_path)
        self.hr_img_path = glob(hr_img_path)
    ...
    def load_images(self):
        lr_images = []
        hr_images = []
        for path in self.lr_img_path:
            image = self.color_imread(path)
            lr_images.append(image)
        for path in self.hr_img_path:
            image = self.color_imread(path)
            hr_images.append(image)
        
        test_img = self.color_imread(self.test_img_path)

        for i in range(len(lr_images)):
          lr_images[i] = self.reshape(lr_images[i])
          hr_images[i] = self.reshape(hr_images[i])
        
        test_img = self.reshape(test_img)
        
        return lr_images , hr_images , test_img
    ...
    def generate_images(self,model, test_input,step):
        ...
        pre = (pre+1)*127.5
        pre = np.uint8(pre)
        name = self.output_img_path+'Colorization_{step}.png'.format(step=step)
        plt.imsave(name,pre)
    ...
    def fit(self):
      ...
      step = 0
      i = 0
      test_img = np.array(test_img).reshape(1,256,256,3)
      input_images = np.array(input_images).reshape(-1,1,256,256,3)
